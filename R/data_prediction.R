...
#' @param project_res A numeric value specifying the resolution of the projected raster in decimal degree. Default is the same as raster layer `domain`.
...
#' @export
project_geoModel <- function(domain, model, output_directory, project_res=NULL, output_name=NULL, fun=function(x,na.rm=TRUE) 1 - prod(1 - x, na.rm=na.rm), save_output=TRUE){
  ...
  # convert the resolution in degree decimal to meters
  #dst <- geosphere::distGeo(p1 = c(0, 0), p2 = c(0, raster::res(domain)[1]))

  # create a regular grid of point pixels
  #pred_grid <- grid_pixels(model@presence, ext=sf::st_bbox(domain), dst=dst)
  #colnames(pred_grid) <- colnames(model@presence)

  # find cells associated to grid points
  #out_cells <- raster::cellFromXY(domain, pred_grid)
  ...
  # out <- domain
  # out[!is.na(out[])] <- 0
  # out[out_cells[!is.na(out_cells)]] <- out_vals

  # mask
  out <- raster::mask(out, domain)

  # dis/aggregate the predictions at finer/coarser scale if needed
  train_res <- round(raster::res(domain)[1], 9)
  if(!is.null(project_res)){
    if(train_res< project_res){
      fac <- ceiling(project_res/train_res)
      out <- raster::aggregate(out, fact=fac, fun=fun)
      domain <- raster::aggregate(domain, fact=fac)
      out <- raster::mask(out, domain)
    }else if(train_res > project_res){
      fac <- ceiling(train_res/project_res)
      out <- raster::disaggregate(out, fact=fac, fun=fun)
      domain <- raster::disaggregate(domain, fact=fac)
      out <- raster::mask(out, domain)
    }
  }
  ...
  if(save_output){
    ...
    raster::writeRaster(out, file.path(output_directory, paste0(output_name,".tif")), overwrite=TRUE)
  }
