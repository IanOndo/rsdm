...
List2MaxEntCommand <- function(argList){
  ...
  for(arg in arg_names){
    ...
    flag = switch(arg,
                  ...
                  'writeclampgrid'	= 	if(!argList[[arg]]) 'dontwriteclampgrid' else '',
                  ...
                  'fadebyclamping'	= 	if(!argList[[arg]]) '' else 'fadebyclamping',

                  'extrapolate'		=	if(!argList[[arg]]) 'dontextrapolate' else '',
                  'visible'			=	if(!argList[[arg]]) '-z' else '',
                  'autofeature'		= 	if(!argList[[arg]]) '-A' else '',
                  'doclamp'			= 	if(!argList[[arg]]) 'nodoclamp' else '',
...
#' Reads a named list and returns an executing command line for MaxEnt
#'
#' @param  maxent_outputdir A character string specifying the path to the output directory where the maxent results are stored.
#' @param eval_metrics A character string of the names of the evaluation metrics to retrieve from the maxent results.
#' @export
get_maxent_output <- function(maxent_outputdir, eval_metrics=c("auc", "omission_rate","tss"), index=1){
  ...
  if("auc" %in% eval_metrics){
    ...
    model.output %<>%
      dplyr::bind_cols(data.frame(AUC_train=auc_train, AUC_test=auc_test, AUC_diff=auc_test-auc_train))
  }
  if("omission_rate" %in% eval_metrics){
    or10_test <- maxentResults[['X10.percentile.training.presence.test.omission']]
    model.output %<>%
      dplyr::bind_cols(data.frame(OR10_test=or10_test))
  }
