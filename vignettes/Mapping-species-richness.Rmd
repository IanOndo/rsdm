...
species_proba_per_cell <- readRDS('path/to/species_proba_per_cell.rds') # this may take a while, so be patient!
...
my_species_ID  <- UsefulPlants:::usefulplants |> # list of useful plants (already ordered by species names)
...
rast_ref <- terra::rast(system.file("extdata","sdm_env_raster_layers.tif",package="UsefulPlants"))[[1]]
...
ext <- raster::rasterFromCells(raster::raster(my_species_distribution), my_species_probas[['cell']][valid.cells], values=FALSE) |>
  terra::extend(c(2,2)) # extend with a number of rows and columns (2 at each side)
terra::crop(my_species_distribution, terra::rast(ext), filename="Acacia_cowleana.tif", overwrite=TRUE)
...
```{r plot-species-richness-maps, warning=FALSE, echo=FALSE, eval=TRUE, include=TRUE, fig.cap="Global distribution of utilised plant species richness. \\textbold{Panel A} shows the number of utilised plant species predicted per 400km^2 grid cell. \\textbold{Panel B} shows the standard deviation.", fig.pos="H", fig.show='hold', fig.align='center'}
...
```{r plot-weighted-endemism, warning=FALSE, echo=FALSE, eval=TRUE, include=TRUE, fig.cap="Global distribution of utilised plant species endemism. the map shows the endemism predicted per 400km^2 grid cell.", fig.pos="H", fig.show='hold', fig.align='center'}
